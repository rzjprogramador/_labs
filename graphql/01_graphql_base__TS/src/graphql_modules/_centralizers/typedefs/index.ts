import { objTypesEntities } from '../entities'
import { objTypesSubEntities } from '../entities/sub_entities'


export const typeDefsSubRoutes = [
  
  objTypesEntities, objTypesSubEntities,

]

/*
ROTEADOR DE TIPE DEFS ->> ALIMENTA O TIPEDEFS ROTEADOR PRINCIPAL

  * REGISTRO DE OBJ TYPEDEFS && SUB OBJTYPEDEFS
  * OBS: ATENCAO USAR SÓ ESTES 2 OBJS PARA PEGAR AS ENTIDADES E SUB ENTIDADES VINDAS DOS SEUS INDEX
  * OBS: SOMENTE ESTES DOIS
*/